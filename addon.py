...
def obtener_eventos_desde_html():
    url = "https://fr.4everproxy.com/direct/aHR0cHM6Ly9jaXJpYWNvLWxpYXJ0LnZlcmNlbC5hcHAv"
    try:
        ...
        if not tabla_eventos:
            xbmc.log("No se encontró la tabla de eventos", xbmc.LOGERROR)
            return []
        for fila in tabla_eventos.find_all('tr')[1:]:
            ...
            if len(columnas) >= 5:
                ...
                for canal in canales_html:
                    ...
                    url = canal.get('href').replace("acestream://", "")  # Solo el ID
                    canales.append({"name": nombre, "url": url})
                if canales:
                    eventos.append({
                        "hora": hora,
                        "categoria": deporte,
        ...
        xbmc.log(f"Se encontraron {len(eventos)} eventos", xbmc.LOGINFO)
        return eventos
    except Exception as e:
        import traceback
        xbmcgui.Dialog().ok("Error AGENDA", str(e), traceback.format_exc())
        xbmc.log(f"Error al obtener eventos: {e}", xbmc.LOGERROR)
        return []
